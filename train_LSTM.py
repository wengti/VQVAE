...
class custom_LSTM_dataset(Dataset):
    ...
    def __getitem__(self, index):
        context, target = self.sents_list[index]
...
# 3. Visualize dataset
print(f"[INFO] Number of available samples: {len(trainDataLSTM)}") # Expected 60,000 x 64 / 10,000 x 64 (10% of them)
...
for i, num in enumerate(randNum):
    ...
    print(f"[INFO] The corresponding target   : {trainDataLSTM[num][1]}")
...
def generate_imgs(LSTM, VQVAE, config, pkl_file, device):
    ...
    with torch.inference_mode():
        ...
        for _ in range(num_samples):
            out = torch.tensor([config['codebook_size']])[None,:].to(device) # 1x1, tensor, float

            for j in range(encoding_length):

                # Pad the test samples until context size is reached
                out_pad = out
                if out_pad.shape[-1] < context_size:
        ...
        out_idx_tensor = torch.cat(out_idx_list, dim=0) # 100 x 1 x 64, converted into tensor form

        h = int(out_idx_tensor.shape[-1] ** 0.5) # Obtain the reshaped shape

        out_idx_tensor = out_idx_tensor.reshape((out_idx_tensor.shape[0], h, h)).long() # 100 x 8 x 8
        out_idx_tensor_one_hot = torch.nn.functional.one_hot(out_idx_tensor,
        ...
        # Pass to VQVAE
        out_imgs = VQVAE.decode_from_codebook_index(out_idx_tensor_one_hot.float().to(device))
...
LSTM1 = custom_LSTM(config = config).to(device)
LSTM1.load_state_dict(torch.load(f = save_lstm_file,
                                 weights_only = True))

VQVAE1 = VQVAE(config = config).to(device)
VQVAE1.load_state_dict(torch.load(f=save_vqvae_file,
                                 weights_only = True))
